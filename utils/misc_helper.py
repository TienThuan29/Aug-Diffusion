...
def save_checkpoint(state, is_best, config, metric_type="default"):
    ...
    if is_best:
        if metric_type == "default":
            ...
            shutil.copyfile(
                os.path.join(folder, "ckpt.pth.tar"),
                os.path.join(folder, f"ckpt_best_{metric_type}.pth.tar"),
            )
...
def load_state(path, model, optimizer=None, return_full_checkpoint=False):
    ...
    if os.path.isfile(path):
        ...
        if optimizer is not None:
            ...
            # Return full checkpoint data when optimizer is None
            return checkpoint
    else:
        if rank == 0:
...
def set_random_seed(seed, reproduce):
    np.random.seed(seed)
