...
"""
    lưu mỗi mẫu trong batch ra một file .npz 
"""
...
"""
    gom toàn bộ các file kết quả .npz trong thư mục save_dir
"""
...
class EvalImageMax(EvalImage):
    ...
    def encode_pred(preds, avgpool_size):
        N, H, W = preds.shape
        # Ensure kernel size does not exceed spatial size to avoid negative outputs
        k_h, k_w = avgpool_size
        k_h = max(1, min(k_h, H))
        k_w = max(1, min(k_w, W))
        device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        preds_t = torch.tensor(preds[:, None, ...], device=device, dtype=torch.float32)  # N x 1 x H x W
        pooled = F.avg_pool2d(preds_t, (k_h, k_w), stride=1)
        pooled = pooled.detach().cpu().numpy()  # N x 1 x H' x W'
        return pooled.reshape(N, -1).max(axis=1)  # (N, )
