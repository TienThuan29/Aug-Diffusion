...
class Downsample(nn.Module):
    def __init__(self, in_channels, use_conv, out_channels=None) -> None:
        super().__init__()
        self.channels = in_channels
        out_channels = out_channels or in_channels
        if use_conv:
            # downsample by 1/2 use conv
    ...
    def forward(self, x, time_embed=None):
        assert x.shape[1] == self.channels # check channels match with input channels
